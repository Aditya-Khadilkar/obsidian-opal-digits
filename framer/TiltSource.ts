// Ported unchanged from src/input/TiltSource.ts, aside from the relative
// import path and the pinned three.js version.
...
/** Everything a debug readout would show. Nothing here is used for rendering. */
...
/**
 * Owns every input path and exposes one smoothed tilt vector in -1..1.
 * Nothing else touches a raw pointer or sensor event.
 *
 * Pointer and touch drag are wired up in every state, including after a
 * denied or missing sensor, so the material is always explorable.
 */
export class TiltSource {
  ...
  private notePointer(): void {
    ...
    // not want to wave their device about can still explore the material.
    this.status = "pointer"
  }
   ...
   * inside a real user gesture handler: iOS rejects requestPermission
   * otherwise, which is why a tap-to-start button is required rather than
   * optional.
   *
   * Resolves to the status the app ended up in. Never throws, and never
   * leaves the page in a state where the material cannot be explored.
   */
  ...
  private readonly onOrientation = (e: Event): void => {
    ...
    // Neutral pose: people hold devices 30 to 60 degrees off flat, so
    // whatever pose the first event reports becomes zero.
    ...
    // Tipping the right edge down turns the slab's right edge away; tipping
    // the top away tips the slab's top away, so beta is negated.
    this.setTarget(
