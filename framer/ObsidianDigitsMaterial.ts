// Ported from src/material/ObsidianDigitsMaterial.ts. The only change is the
// shader source: Framer has no build step to run vite-plugin-glsl-include, so
// the shaders come from the pre-resolved strings in ./shaders instead of raw
// .vert/.frag imports.

import * as THREE from "three@0.169.0"
import { mainVertexShader as vertexShader, mainFragmentShader as fragmentShader } from "./shaders"
 ...
 * them, which is what sweeps the hue; see the note on uLightDirs in the
 * fragment shader.
 *
...
export class ObsidianDigitsMaterial extends THREE.ShaderMaterial {
  ...
  /**
   * Copies params into uniforms. Called once per frame.
   *
